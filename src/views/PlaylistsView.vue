<template>
  <div class="playlists-page">
    ...
    <section class="playlist-hero">
      ...
      <div class="hero-content">
        ...
        <div class="hero-info">
          ...
          <p class="hero-meta">
            <span class="meta-item">{{ playlist.songs?.length || 0 }} canciones</span>
      ...
      <div class="hero-actions">
        <button class="btn-play-all" @click="playAll" :disabled="!playlist.songs?.length">
          <i class="bi bi-play-fill"></i>
        </button>
        <button class="btn-shuffle" @click="shufflePlay" :disabled="!playlist.songs?.length">
    ...
    <!-- Songs List -->
    <section class="songs-section" v-if="playlist.songs?.length > 0">
      ...
      <div class="songs-list">
        ...
        >
          ...
          <div class="col-actions">
            ...
            <button class="action-btn remove" @click="removeSong(song.id)" title="Quitar de playlist">
    ...
    <section class="empty-state" v-else>
      ...
      <p>Añade canciones desde el buscador o desde las páginas de artistas</p>
...
const formatTotalDuration = () => {
  if (!playlist.value?.songs) return '0 min';
  const totalSeconds = playlist.value.songs.reduce((acc, song) => acc + (song.duration || 0), 0);
...
const playSong = (song) => {
  store.playWithQueue(song, playlist.value.songs);
};

const playAll = () => {
  if (playlist.value?.songs?.length > 0) {
    store.playWithQueue(playlist.value.songs[0], playlist.value.songs);
  }
};
...
const removeSong = (songId) => {
  ...
  showToastMessage('Eliminado de la playlist');
};
...
const toggleFavorite = (song) => {
  if (isFavorite(song)) {
    ...
    showToastMessage('Añadido a Me Gusta');
  }
};
...
$deezer-pink: #EF5466;
$deezer-gradient: linear-gradient(135deg, #EF5466 0%, #A238FF 100%);
$bg-dark: #121212;
$bg-card: #181818;
$bg-hover: #282828;
$text-primary: #ffffff;
...
.playlist-hero {
  ...
  background: linear-gradient(180deg, rgba(239, 84, 102, 0.4) 0%, $bg-dark 100%);
}
...
.hero-gradient {
  ...
  right: 0;
  bottom: 0;
  background: radial-gradient(ellipse at top left, rgba($deezer-pink, 0.3) 0%, transparent 50%);
  pointer-events: none;
}
...
.btn-play-all {
  ...
  box-shadow: 0 8px 20px rgba($deezer-pink, 0.4);
...
.songs-header {
  display: grid;
  grid-template-columns: 50px 4fr 3fr 100px 130px;
  gap: 16px;
  padding: 12px 16px;
...
.song-row {
  ...
  &.playing {
    background: rgba($deezer-pink, 0.15);
  }
}
...
.btn-explore {
  ...
  &:hover {
    ...
    box-shadow: 0 8px 25px rgba($deezer-pink, 0.4);
  }
}
